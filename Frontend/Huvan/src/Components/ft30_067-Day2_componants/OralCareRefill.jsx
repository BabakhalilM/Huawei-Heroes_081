...
const Card = ({ image, image2, title, badge, rating, price, buttonText }) => {
  ...
  return (
    ...
    >
      <div className="card-content">
        <img onClick={handleNavigate} src={currentImage} alt={title} className="card-img" />
        <h3 style={{fontWeight:"normal",fontFamily:"cursive"}} className="card-title">{title}</h3>
        {rating && <p className="card-rating">Rating:{rating}</p>}
        <p className="card-price">${price}</p>
        <button onClick={handleAddToCart} className="card-button">{buttonText}</button>
      </div>
    </div>
  );
};

export const OralCareRefill = () => {
  const products =[
    {
    ...
    <>
      <div className="firstimg">
        <img
          className="fimg"
          style={{height:'60vh'}}
          src="https://haanready.com/cdn/shop/collections/Desktop_Oral_Care_1_6fb1dadc-df7a-4cd6-b8c1-3c6469c3d186.jpg?v=1710955603&width=1920"
          alt=""
        />
        <p
          style={{
